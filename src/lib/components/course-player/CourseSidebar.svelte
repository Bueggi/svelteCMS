                    ...
                    {#each module.lessons as lesson (lesson.id)}
                        ...
                        >
                            <div class={cn(
                                ...
                                isActive ? "bg-gradient-to-r from-primary/5 to-transparent opacity-100" : ""
                            )}></div>
                            ...
                            <div class="relative z-10 flex-1 min-w-0">
                                ...
                                {#if isLocked && daysLeft != null}
                                    <span class="inline-flex items-center gap-1 mt-1 text-[10px] font-medium px-1.5 py-0.5 rounded bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-700">
                                        ...
                                        {daysLeft <= 0 ? 'Bald verfügbar' : daysLeft === 1 ? 'in 1 Tag' : `in ${daysLeft} Tagen`}
                                    </span>
                                {/if}
                            </div>
                        </a>
    ...
    {#if !preview}
        <div class="p-4 border-t bg-card/50">
           <div class="text-xs text-center text-muted-foreground">
               {Math.round((completedLessonIds.size / course.modules.reduce((acc, m) => acc + m.lessons.length, 0)) * 100)}% Complete
           </div>
           <div class="h-1 w-full bg-secondary/20 mt-2 rounded-full overflow-hidden">
               <div class="h-full bg-primary transition-all duration-500" style={`width: ${(completedLessonIds.size / course.modules.reduce((acc, m) => acc + m.lessons.length, 0)) * 100}%`}></div>
           </div>
        </div>
    {:else}
