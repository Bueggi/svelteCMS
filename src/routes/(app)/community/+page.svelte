...
<PageContainer>
    <PageHeader title={t('communityTitle')} description={t('communityDesc')} />
            ...
            {#each communities as community}
                ...
                >
                    <div class="aspect-video relative overflow-hidden bg-muted">
                        ...
                        />
                        <div class="absolute inset-0 bg-linear-to-t from-black/70 to-transparent flex items-end p-5">
                    ...
                    <div class="px-5 py-4 flex items-center gap-5 text-sm text-muted-foreground border-t border-border/40">
                        <span class="flex items-center gap-1.5">
                            <Users class="w-4 h-4" /> {t('communityMembers')}
                        </span>
                        <span class="flex items-center gap-1.5">
                            <MessageSquare class="w-4 h-4" /> {t('communityDiscussions')}
                        </span>
                    </div>
                </a>
            {/each}
    ...
    {:else}
        <div class="flex flex-col items-center justify-center py-20 text-center bg-card border border-dashed border-border rounded-2xl">
            ...
            <h2 class="text-xl font-semibold text-foreground mb-2">{t('communityNone')}</h2>
            <p class="text-muted-foreground max-w-sm mb-6 text-sm">
                {t('communityNoneDesc')}
            </p>
            <Button href="/courses">{t('communityBrowse')}</Button>
        </div>
    {/if}
</PageContainer>
