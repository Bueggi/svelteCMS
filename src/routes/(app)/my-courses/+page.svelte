...
<PageContainer>
    <PageHeader title="Meine Kurse" description="Mach weiter, wo du aufgehört hast." />
                    ...
                    <div class="relative aspect-video overflow-hidden shrink-0">
                        ...
                        {#if isCompleted}
                            <div class="absolute top-3 left-3 flex items-center gap-1.5 bg-emerald-500 text-white text-[11px] font-semibold px-2.5 py-1 rounded-full shadow">
                                <CheckCircle2 class="w-3 h-3" /> Abgeschlossen
                            </div>
                        {:else}
                    ...
                    <div class="flex-1 p-5 flex flex-col gap-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="h-1.5 bg-muted rounded-full overflow-hidden">
                                <div
                                    class="h-full rounded-full transition-all duration-500 {isCompleted ? 'bg-emerald-500' : 'bg-gradient-to-r from-primary to-primary/70'}"
                                    style="width: {course.progress}%"
                                ></div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between pt-1 border-t border-border/40">
                            <span class="text-sm font-medium text-primary group-hover:underline underline-offset-2">
                                {isCompleted ? 'Wiederholen' : isStarted ? 'Weiter lernen' : 'Kurs starten'}
                            </span>
    ...
    {:else}
        <div class="flex flex-col items-center justify-center py-24 text-center">
            ...
            <h2 class="text-2xl font-serif font-semibold text-foreground mb-2">Noch keine Kurse</h2>
            <p class="text-muted-foreground max-w-sm mb-8 leading-relaxed">
                Entdecke unseren Kurskatalog und starte deinen ersten Kurs.
            </p>
            <a
                href="/courses"
            ...
            >
                Kurse entdecken <ArrowRight class="w-4 h-4" />
            </a>
        </div>
    {/if}
</PageContainer>
